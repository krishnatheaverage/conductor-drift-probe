...
important such cell, the safety subspace in SAE feature space on an instruction-tuned chat model,
is now complete: on Gemma-2-2B-it with Gemma Scope (\citealp{lieberum2024gemmascope}; run in
bfloat16 on an L4/A100, after a \texttt{transformer\_lens} plus Gemma-2 load issue ruled out the
T4), both the hallucination and the safety subspace return the null (Table~\ref{tab:gemma}). The
remaining pending cells are a mixture-of-experts model, higher-rank subspaces, a within-topic
content control, and a behavioral static-projector test.
...
Gemma-2-2B-it, SAE space, hallucination + safety & ``SAE safety side on a chat model'' & done \\
...
\bottomrule
\end{tabular}
\caption{Evaluation matrix. Raw-activation replication across two model scales, the SAE
feature-space replication (GPT-2 and Gemma-2-2B-it, the latter for both the hallucination and
the safety subspace), and the static-projector demonstration are complete. The remaining cells
(mixture-of-experts, higher rank, within-topic control, behavioral test) are pending; the
protocol ports to each with only a backend swap.}
\label{tab:matrix}
\end{table}
