...
\paragraph{Replication on an instruction-tuned model, both subspaces (Gemma-2-2B-it).} The
settings above leave one gap: the safety/refusal subspace, poorly recoverable in the small raw
models, and an instruction-tuned model in SAE feature space. We close both with Gemma-2-2B-it and
the Gemma Scope residual SAEs (width $16$k) at seven layers, testing the hallucination
\emph{and} the safety subspace (Table~\ref{tab:gemma}). Both return
\textsc{not\_content\_conditional}: no layer clears the rule for either subspace, and as in GPT-2
the scrambled twin matches or exceeds the real spread at several mid layers (hallucination layer
$6$: real $0.765$ $[0.70,0.85]$ versus scrambled $0.870$ $[0.84,0.93]$; layer $9$: $0.787$ versus
$0.855$). The $16{,}384$-dimensional SAE feature space has a higher estimation-noise floor (wider
CIs, spreads of $0.4$--$0.9$), so the test is less powered here; the finding is the absence of any
real-over-scrambled gap, not a tight bound. Caveat: the public Gemma Scope SAEs are trained on the
base model, so applying them to the instruction-tuned model is a mild distribution mismatch and
...
\begin{tabular}{@{}lcccc@{}}
\toprule
& \multicolumn{2}{c}{Hallucination subspace} & \multicolumn{2}{c}{Safety subspace} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Layer & Real $[95\%\ \text{CI}]$ & Scrambled $[95\%\ \text{CI}]$ & Real $[95\%\ \text{CI}]$ & Scrambled $[95\%\ \text{CI}]$ \\
\midrule
$3$  & $0.812$ $[0.73,0.86]$ & $0.790$ $[0.74,0.87]$ & $0.612$ $[0.53,0.87]$ & $0.587$ $[0.47,0.75]$ \\
$6$  & $0.765$ $[0.70,0.85]$ & $0.870$ $[0.84,0.93]$ & $0.961$ $[0.92,1.00]$ & $0.922$ $[0.76,1.00]$ \\
$9$  & $0.787$ $[0.71,0.84]$ & $0.855$ $[0.82,0.91]$ & $0.616$ $[0.49,0.68]$ & $0.677$ $[0.58,0.87]$ \\
$12$ & $0.852$ $[0.78,0.92]$ & $0.888$ $[0.82,0.92]$ & $0.613$ $[0.55,0.72]$ & $0.539$ $[0.46,0.68]$ \\
$15$ & $0.561$ $[0.50,0.72]$ & $0.484$ $[0.43,0.58]$ & $0.512$ $[0.46,0.61]$ & $0.433$ $[0.37,0.50]$ \\
$18$ & $0.543$ $[0.41,0.61]$ & $0.416$ $[0.35,0.48]$ & $0.437$ $[0.38,0.50]$ & $0.390$ $[0.32,0.46]$ \\
$21$ & $0.481$ $[0.37,0.54]$ & $0.325$ $[0.27,0.42]$ & $0.402$ $[0.36,0.50]$ & $0.336$ $[0.27,0.40]$ \\
\bottomrule
\end{tabular}}
\caption{Instruction-tuned SAE-feature-space replication on Gemma-2-2B-it (Gemma Scope
width-$16$k residual SAEs, $n{=}12$, rank-1). Both the hallucination and the safety/refusal
subspace return \textsc{not\_content\_conditional}: no layer has the real spread's CI lower bound
above the scrambled spread's CI upper bound. This is the only setting in which the safety subspace
is recoverable enough to test, and it too shows no content-conditionality.}
\label{tab:gemma}
\end{table}
...
\paragraph{Summary across settings.} Every configuration tested under the full protocol returns
\textsc{not\_content\_conditional}: \texttt{Qwen2.5-0.5B-Instruct} and
\texttt{Qwen2.5-1.5B-Instruct} in raw activation space, GPT-2 small in SAE feature space, and
\texttt{Gemma-2-2B-it} in SAE feature space for \emph{both} the hallucination and the
safety/refusal subspace; the static-projector demonstration returns
\textsc{static\_failure\_is\_confound}. Across these models, in raw and SAE feature space, and for
both subspaces, the apparent context-conditionality is statistically indistinguishable from a
